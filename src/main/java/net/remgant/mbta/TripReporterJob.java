 ...
 */
public class TripReporterJob {
    Logger log = LoggerFactory.getLogger(TripReporterJob.class);
    ...
    public void processTripData() {
        for (Trip trip : trips) {
            ...
            if (data == null || data.size() == 0)
                continue;
            List<Object[]> schedule = stopTimesDAO.getSchedule(tripId);
            ...
            Object sched[] = schedule.get(stopSequence - 1);
            LocalTime localTime = (LocalTime)sched[1];
            ...
            onTimeDataDAO.addRecord(LocalDate.now(), Instant.now(), tripId, sched[0].toString(), stopSequence,
